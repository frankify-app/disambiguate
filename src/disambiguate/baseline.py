...
from it fail the run. The baseline only shrinks: an entry whose finding no
longer occurs is reported as a fatal `stale-baseline` finding so it is
removed deliberately, rather than rotting into a standing excuse.
...
reported); chosen: reported, and fatal. Auto-pruning made a read-only
command write a tracked file, and in CI that write is discarded with the
runner's checkout — so the baseline shrank nowhere that lasts. Reporting
puts the shrink in a commit and matches how a stale suppression is
treated. The baseline file is `.drift-baseline.json` next to
pyproject.toml (else repo root); regeneration is
`--drift --write-baseline`.
"""
